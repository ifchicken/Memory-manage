...
//                             auto childNode = std::find_if(_nodes.begin(), _nodes.end(), [&childToken](GraphNode *node) { return node->GetID() == std::stoi(childToken->second); });
                            ...
                            // transfer the ownership to each node
...
//     rootNode->MoveChatbotHere(_chatBot);
    ...
    // stack instance of ChatBot
    ChatBot localBot = ChatBot("../images/chatbot.png");

    // member variable have the current address of the bot
    _chatBot = &localBot;   
 
  
    // add pointer to chatlogic so chatbot answers can be passed on to the GUI
